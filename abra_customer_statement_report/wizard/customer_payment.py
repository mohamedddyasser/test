...
class CustomerPayment(models.TransientModel):
    ...
    account_move_ids = fields.Many2many(comodel_name="account.move", string="Customer Invoice")
    ...
    def button_print(self):
        ...
        if account_move:
            for invoice in account_move:
                ...
                total_amount_currency+=invoice.currency_id._convert(invoice.amount_residual, self.env.user.company_id.currency_id, self.env.user.company_id, fields.Date.context_today(self))
                ...
                account_move_date.append({
                    ...
                    "amount_residual":str(invoice.amount_residual) +str(" ")+str(invoice.currency_id.name),
                })
        ...
        wizard_data.append({
            ...
            'total_amount_currency': str(self.env.user.company_id.currency_id.name)+" "+str(total_amount_currency),
        ...
        return (self.env.ref('abra_customer_statement_report.customer_payment_report_pdf').
...
class CustomerPaymentReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        report = report_obj._get_report_from_name('abra_customer_statement_report.customer_payment_report')
