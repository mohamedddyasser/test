    ...
    def button_print(self):
        ...
        if account_move:
            for invoice in account_move:
                ...
                dif_date=datetime.strptime(str(invoice.invoice_date_due), '%Y-%m-%d').date()- date.today()

                amount_currency=invoice.currency_id._convert(invoice.amount_residual, self.env.user.company_id.currency_id, self.env.user.company_id, fields.Date.context_today(self))

                total_amount_currency +=amount_currency

                # print(amount_currency,"$$$$$$$$$$$$$$$$$$$$$$")
                account_move_date.append({
                    "invoice_date":datetime.strptime(str(invoice.invoice_date), '%Y-%m-%d').strftime('%d-%b-%Y') if invoice.invoice_date else "",
                    "name":invoice.name,
                    "ref":invoice.ref,
                    ...
                    "amount_currency":str(invoice.amount_residual)+str(" ")+str(invoice.currency_id.name),
                    "invoice_date_due": int(dif_date.days),
                    "due_in":datetime.strptime(str(invoice.invoice_date_due), '%Y-%m-%d').strftime('%d-%b-%Y') if invoice.invoice_date_due else "",

                })
        wizard_data.append({
            'date': datetime.strptime(str(self.date), '%Y-%m-%d').strftime('%d-%b-%Y') if self.date else "",
        ...
        return (self.env.ref('abra_customer_statement_report.customer_statement_report_pdf').
...
class CustomerStatementReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        report = report_obj._get_report_from_name('abra_customer_statement_report.customer_statement_report')
