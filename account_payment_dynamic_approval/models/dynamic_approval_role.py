...
class DynamicApprovalRole(models.Model):
    ...
    def get_approval_user(self, model, res):
        ...
        if (
            model == "sale.order"
            and res.vertical_analytic_account_id
            and res.vertical_analytic_account_id.apply_approval_role
        ):
            analytic_approval_role = (
                res.vertical_analytic_account_id.approval_role_ids.filtered(
