...
class AccountMove(models.Model):
    ...
    payment_counter = fields.Integer(string="Payments", compute='get_payments_count', )
    ...
    reconciled_invoice_payments_ids = fields.Many2many('account.payment', string="Reconciled Payments",
                                                       relation='account_payment_bill_reconciled_rel',
                                                       help="Bills with journal items reconciled with this payments.")

    reconciled_bill_payments_ids = fields.Many2many('account.payment', string="Reconciled Payments",
                                                    relation='account_payment_bill_reconciled_rel',
    ...
    def get_payments_reconciled(self):
        ...
        return {
            ...
            'view_mode': 'tree,form',
            'target': 'current',
            'domain': [('id', 'in', (self.reconciled_bill_payments_ids | self.reconciled_bill_payments_ids).ids)],
        }
    ...
    def get_reconciled_payments_count(self):
        for rec in self:
            rec.reconciled_payments_count = len(rec.reconciled_bill_payments_ids | rec.reconciled_bill_payments_ids)
    ...
    def _compute_name(self):
        ...
        for move in self:
            ...
            if ((move_has_name or move.state != 'posted') and move.move_type != 'out_invoice') or ((move_has_name or move.state != 'under_approval') and move.move_type == 'out_invoice'):
    ...
    def get_payments_count(self):
        for rec in self:
            rec.payment_counter = len(self.env['account.payment'].search([
                ('partner_id', '=', rec.partner_id.id), ('is_reconciled', '=', False)
            ]))
    ...
    def get_payments_created(self):
        return {
            ...
            'view_mode': 'tree,form',
            'target': 'current',
            'domain': [('partner_id', '=', self.partner_id.id), ('is_reconciled', '=', False)],
        }

    def _get_under_validation_exceptions(self):
        exception_list = super()._get_under_validation_exceptions()
        if self._name == 'account.move':
            move_exception_list = ['needed_terms_dirty', 'show_original_buttons', 'custom_is_confirm', 'warn_msg',
                                   'invoice_line_ids', 'line_ids','tax_totals','amount_untaxed','amount_tax', 'name'
                                   'amount_total','amount_residual','amount_untaxed_signed','amount_tax_signed',
                                   'amount_total_signed','amount_residual_signed','amount_total_in_currency_signed',
                                   'needed_terms_dirty', 'sequence_prefix', 'sequence_number']
    ...
    def _get_after_validation_exceptions(self):
        ...
        if self._name == 'account.move':
            move_exception_list = ['needed_terms_dirty', 'show_original_buttons', 'custom_is_confirm', 'warn_msg', 'message_main_attachment_id', 'is_move_sent', 'access_token', 'send_and_print_values']
            exception_list.extend(move_exception_list)
        return exception_list
    ...
    def button_cancel(self):
        return super(AccountMove, self.with_context(force_dynamic_validation=True)).button_cancel()

    def button_draft(self):
        return super(AccountMove, self.with_context(force_dynamic_validation=True)).button_draft()
