...
class WorkFlowSelect(models.TransientModel):
    ...
    def compute_approval_ids(self):
        ...
        company = getattr(record, record._company_field) if record._company_field else False
    ...
    def action_apply(self):
        record = self.env[self.res_model].browse(int(self.res_id))
        approval_request_values = self.approval_id._prepare_approval_request_values(self.res_model, record)
