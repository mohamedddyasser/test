...
class DynamicApprovalWizard(models.TransientModel):
    ...
    dynamic_approval_id = fields.Many2one(
        comodel_name="dynamic.approval", string="Dynamic Approval"
    )
    ...
    record = fields.Integer(string="Record", required=False)

    def action_set_approvers(self, manager=None):
        record = self.env[self._context.get("active_model")].browse(
            self._context.get("active_id")
        )
        matched_approval = self.dynamic_approval_id

        approval_request_values = self._prepare_approval_request_values_wiz(
            self._context.get("active_model"), record
        )
        if approval_request_values:
            pass
        else:
            raise AccessError(
                "Dynamic Approval Levels must have one Approver User at least."
            )
    ...
    def _prepare_approval_request_values_wiz(self, model, res):
        ...
        self.ensure_one()
        return [
            level.prepare_approval_request_values_wiz(model=model, res=res)
            for level in self.dynamic_approval_level_id
        ]
    ...
    def default_get(self, fields):
        ...
        find_model = (
            self.env["ir.model"]
            .sudo()
            .search([("model", "=", self._context.get("active_model"))])
            .id
        )
        rec = (
            self.env["dynamic.approval"].sudo().search([("model_id", "=", find_model)])
        )
        active_rec = (
            self.env[self._context.get("active_model")]
            .sudo()
            .browse(int(self._context.get("active_id")))
        )
        ...
        if rec.line_manager_approval:
            ...
            if rec.line_manager_approval and not employee.user_id.has_group(
        ...
        res.update(
            {
                "dynamic_approval_level_id": approval_val_list,
...
class DynamicApprovalLevelWizard(models.TransientModel):
    ...
    def onchange_method(self):
        if (
            self.define_levels_id
            and not self.define_levels_id.dynamic_approval_id.self_approval
        ):
            active_rec = (
                self.env[self._context.get("active_model")]
                .sudo()
