...
class DynamicApprovalRequest(models.Model):
    ...
    def _compute_res_name(self):
        ...
        for record in self:
            record.res_name = (
                record.res_model
                and self.env[record.res_model].browse(record.res_id).display_name
            )
    ...
    def action_send_reminder_email(self):
        ...
        calendar_id = (
            ...
            .sudo()
            .get_param("base_dynamic_approval.email_resource_calendar_id", 0)
        )
        calendar_id = cal_obj.search([("id", "=", calendar_id)])
        for record in self:
            if (
                record.dynamic_approval_id
                and record.dynamic_approval_id.reminder_pending_approver_email_template_id
            ):
    ...
    def _cron_send_reminder_to_approve(self):
        ...
        approve_requests = self.search(
            [
                ("status", "=", "pending"),
                (
                    "dynamic_approval_id.reminder_pending_approver_email_template_id",
                    "!=",
                    False,
                ),
            ]
        ...
        for approve_request in approve_requests:
            if approve_request.dynamic_approval_id.reminder_period_to_approve:
                delay_period = (
                    approve_request.dynamic_approval_id.reminder_period_to_approve
                )
                request_end_date = (
                    approve_request.last_reminder_date or approve_request.write_date
                )
