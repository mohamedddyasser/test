...
class MailThread(models.AbstractModel):
    ...
    def _document_check_access_per_user(self, model_name, document_id, user_id, access_token=None):
    ...
    def _submit_approval_action(self, alias_id, id, user, action, action_note=None):
        record_id = self._document_check_access_per_user(alias_id.alias_model_id.model, int(id), user)
                ...
                for alias in dest_aliases:
                    ...
                    if alias.apply_dynamic_approval:
                        try:
                                ...
                                for i in body.split():
                                    if re.search(r"(?<=\[).+?(?=\])", i):
                                        ...
                                        if id.isdigit():
                                            ...
                                            break
                                if not obj_id:
                                    raise ValidationError(_("Can't get value for object ID"))
                                action_note = ""
                                note_section = body.split("[[NOTE]]")
                                note_section = note_section and len(note_section) > 1 and note_section[1]
                                if note_section:
                                    end_note = re.sub("<.*?>", "", note_section).split("[[ENDNOTE]]")
                                    if end_note:
                                        action_note = end_note[0].replace('\n', '')
                                action = body.find('approve') > 0 and 'approve' or 'reject'
                                self._submit_approval_action(alias, obj_id, user_id, action, action_note)
                            ...
                            _logger.info("exception during processing approval email %s" % e)
                            _logger.info("Can't apply the approval cycle for the message" + str(
                                html.unescape(message_dict['body'])))
                            return
                    route = (
                        alias.sudo().alias_model_id.model, force_thread_id, ast.literal_eval(alias.alias_defaults),
                        user_id, alias)
