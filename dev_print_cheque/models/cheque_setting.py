...
class cheque_setting(models.Model):
    ...
    partner_text = fields.Selection(
        [("prefix", "Prefix"), ("suffix", "Suffix")], string="Partner Title"
    )
    ...
    date_formate = fields.Selection(
        [("dd_mm", "DD MM"), ("mm_dd", "MM DD")], string="Date Formate", default="dd_mm"
    )
    year_formate = fields.Selection(
        [("yy", "YY"), ("yyyy", "YYYY")], string="Year Format", default="yy"
    )
    ...
    is_star = fields.Boolean(
        "Print Star",
        help="if true then print 3 star before and after Amount",
        default=True,
    )
    ...
    is_star_word = fields.Boolean(
        "Word Print Star",
        help="if true then print 3 star before and after Words Amount",
        default=True,
    )

    breaks_number = fields.Integer(
        "Breaks Number",
        help="Numbers of break line after every line of Amount in Word",
        default=1,
    )
    ...
    def _check_description(self):
        for line in self:
            if line.set_default:
                line_ids = self.env["cheque.setting"].search(
                    [
                        ("set_default", "=", True),
                ...
                if len(line_ids) > 1:
                    raise ValidationError(
                        "One Company have one default cheque template"
                    )
