...
class AccountMoveInherit(models.Model):
    ...
    credit_move_id = fields.Many2one(comodel_name="account.move",compute='compute_credit_move')
    ...
    def compute_credit_move(self):
        for rec in self:
            ...
            if rec.move_type=='out_refund':
                account_move=self.env['account.move'].search([('reversal_move_id','in',rec.id),('move_type','=','out_invoice')])
                if account_move:
    ...
    def get_views(self, views, options=None):
        ...
        if options.get('action_id') and self.env['ir.actions.act_window'].sudo().browse(
                options.get('action_id')).xml_id != "account.action_move_out_refund_type":

            def get_report_id():
                return self.env['ir.model.data']._xmlid_to_res_id(
                    "abra_credit_note.account_tax_credit_note_report", raise_if_not_found=False)
