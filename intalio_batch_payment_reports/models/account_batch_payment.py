...
class AccountBatchPayment(models.Model):
    ...
    def account_batch_payment_report_excel_1(self):
        data = {}
        return self.env.ref(
            "intalio_batch_payment_reports.action_batch_payment_xlsx_report1"
        ).report_action(self, data=data)

    def account_batch_payment_report_excel_2(self):
        data = {}
        return self.env.ref(
            "intalio_batch_payment_reports.action_batch_payment_xlsx_report2"
        ).report_action(self, data=data)

    def account_batch_payment_report_excel_3(self):
        data = {}
        return self.env.ref(
            "intalio_batch_payment_reports.action_batch_payment_xlsx_report3"
        ).report_action(self, data=data)
...
class ReportXlsxAbstract1(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batch_payment):
        ...
        sheet.merge_range(
            1, 0, 1, 6, data="Template starts from ROW 3 below", cell_format=head
        )
...
class ReportXlsxAbstract3(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, batch_payment):
        ...
        cell_format = workbook.add_format(
            {"font_name": "Calibri", "bold": True, "font_size": "11px"}
        )
